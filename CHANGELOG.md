...
* **Friendly Boss NPC Frames** — fix only one frame being visible when multiple friendly boss units exist. Previous alpha tried to compact visible frames by closing empty slots, but that required repositioning during combat which isn't allowed on secure frames — causing all visible frames to stack at the same position. Reverted to fixed-slot positioning (boss1 → slot 1, boss2 → slot 2, etc.) matching ElvUI's boss frame layout. Empty gaps can appear when some bosses are hostile (e.g. friendly boss3 shows at slot 3 even if slots 1–2 are empty) but every visible boss is always shown.
...
* **Friendly Boss NPC Frames** — Pinned frame sets now have a Frame Type setting. Switch a set to "Friendly Boss NPCs" to display healable friendly boss units (boss1–boss8) instead of group members. Useful for encounters where friendly adds need healing. All layout, positioning, click-casting, buffs, debuffs, Aura Designer indicators, and out-of-range fading work the same as player-mode pinned sets. Frames use fixed-slot positioning by boss unit index (boss1 → slot 1, etc.) to match ElvUI's reliable multi-boss display.
